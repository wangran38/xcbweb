<template>
    <div class="app-container">
        ...
        <div class="filter-container" style="margin:0 0 2% 0;" v-if="searchSeen">
            <el-input v-model="listQuery.phone" placeholder="输入手机号" style="width: 200px;" class="filter-item"
                @keyup.enter.native="handleFilter" />
            <el-input v-model="listQuery.username" placeholder="输入名称" style="width: 200px;" class="filter-item"
                @keyup.enter.native="handleFilter" />
        ...
        <el-table v-load="orderListLoading" :data="list" border fit highlight-current-row style="width: 100%;"
            ...
            <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
                <template slot-scope="{row,$index}">
                    ...
                    <el-button v-if="row.status != 'published'" size="big" type="success" @click="clockinDetail(row)">
                        打卡明细({{ row.signnum }})
                    </el-button>
                </template>
            </el-table-column>
            <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
                <template slot-scope="{row,$index}">
                    ...
                    <el-button v-if="row.status != 'published'" size="big" type="success" @click="awardDetail(row)">
                        中奖明细({{ row.lotterynum }})
                    </el-button>
                </template>
            </el-table-column>
        ...
        <el-drawer :title="'打卡明细'" :visible.sync="drawer1" direction="rtl" size="50%" @close="closeDrawer">
            <!-- <div style="display: flex; margin: 5px 5px; align-items: center;">
                <div style="font-size: 18px;">支付方式</div>
                <el-select v-model="orderListQuery.pay_way" placeholder="Select"
            ...
            <h2>累计打卡次数：{{ listQuery1CorrelationTotal }}次</h2>
            <el-table :key="tableKey" v-loading="listLoading" :data="listQuery1CorrelationData" border fit
                highlight-current-row style="width: 100%;" @sort-change="sortChange" height="400">
            ...
            <pagination v-show="listQuery1CorrelationTotal > 0" :total="listQuery1CorrelationTotal"
                :page.sync="listQuery1.page" :limit.sync="listQuery1.limit" @pagination="getClockinData" />
        </el-drawer>

        <el-drawer :title="'中奖明细'" :visible.sync="drawer2" direction="rtl" size="50%" @close="closeDrawer">
            <!-- <div style="display: flex; margin: 5px 5px; align-items: center;">
                <div style="font-size: 18px;">支付方式</div>
                <el-select v-model="orderListQuery.pay_way" placeholder="Select"
                    style="width: 240px;margin: 0 0 0 10px;">
            ...
            <h2>累计中奖次数：{{ listQuery2CorrelationTotal }}次</h2>
            <el-table element-loading-text="拼命加载中" element-loading-spinner="el-icon-loading"
                :data="listQuery2CorrelationData" border style="width: 100%;" :stripe="true" empty-text="暂无数据"
                height="400">
                ...
                <el-table-column label="等级" width="100" align="center">
                    <template slot-scope="{row}">
                        <span v-if="row.type == 1">一等奖</span>
                        <span v-if="row.type == 2">二等奖</span>
                        <span v-if="row.type == 3">三等奖</span>
                    </template>
                </el-table-column>
                <el-table-column label="开奖时间" min-width="100px" align="center">
                    <template slot-scope="{row}">
            ...
            <pagination v-show="listQuery2CorrelationTotal > 0" :total="listQuery2CorrelationTotal"
                :page.sync="listQuery2.page" :limit.sync="listQuery2.limit" @pagination="getAwardData" />
        </el-drawer>
...
import { getsignlist, getAwardList } from '@/api/usersign/list'
...
export default {
    ...
    data() {
        return {
            ...
            listQuery1CorrelationData: [],
            listQuery2CorrelationData: [],
            listQuery1CorrelationTotal: 0,
            listQuery2CorrelationTotal: 0,
    ...
    methods: {
        getClockinData() {
            this.listQuery1.user_id = this.selectItemId
            getsignlist(this.listQuery1).then(response => {
                this.listQuery1CorrelationData = response.data.listdata
                this.listQuery1CorrelationTotal = response.data.totalnum
            })
        },
        getAwardData() {
            this.listQuery2.user_id = this.selectItemId
            getAwardList(this.listQuery2).then(response => {
                this.listQuery2CorrelationData = response.data.listdata
                this.listQuery2CorrelationTotal = response.data.totalnum
            })
        },
        closeDrawer() {
            console.log("重置参数")
